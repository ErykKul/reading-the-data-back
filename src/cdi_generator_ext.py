...
class ColumnStats:
    ...
    __slots__ = (
        ...
        "could_be_bool",
        "could_be_datetime",
        # --- P2 extension: evidence for the distribution KIND ---
        "n_numeric", "vmin", "vmax", "n_negative", "n_unit_interval",
        "sum1", "sum2", "sum3", "small_values",
    )
    ...
    def __init__(self, name: str):
        ...
        self.vmax = None
        self.n_negative = 0
        self.n_unit_interval = 0
        self.sum1 = 0.0
        self.sum2 = 0.0
        self.sum3 = 0.0
        self.small_values = set()
    ...
    def update(self, raw: Optional[str]):
        ...
        # --- P2 extension: numeric evidence for distribution kind ---
        try:
            x = float(s)
        except ValueError:
            return
        ...
        if x < 0:
            self.n_negative += 1
        if 0.0 <= x <= 1.0:
            self.n_unit_interval += 1
        self.sum1 += x
        self.sum2 += x * x
        self.sum3 += x * x * x
        if len(self.small_values) <= 25:
            self.small_values.add(x)
    ...
    def distribution_kind(self) -> str:
        """P2 extension: classify the outcome distribution KIND from streaming stats,
        distinguishing binary / ordinal / count / proportion / heavy-tailed / continuous --
        the resolution the XSD Integer/Double datatype cannot provide, and the facet the
        model-data mismatch operator matches against the paper fingerprint's assumed distribution."""
        n = self.n_numeric
        if n == 0:
            return "categorical"
        distinct = self.approx_distinct()
        mean = self.sum1 / n
        var = max(self.sum2 / n - mean * mean, 1e-12)
        sd = var ** 0.5
        # SIGNED skew (positive = right-skewed, the signature of an event count)
        skew = (self.sum3 / n - 3 * mean * var - mean ** 3) / (sd ** 3) if sd > 0 else 0.0
        is_int = self.could_be_int
        # Sentinel-aware low-cardinality handling: survey missing-value codes (98, 99, -99, ...) inflate a
        # Likert/rating scale into a spurious "count"; strip them and read the true small ordinal scale.
        SENT = {98, 99, -1, -7, -8, -9, -99, -998, -999, 999, 9998, 9999, -9998, -9999}
        vals = self.small_values
        if is_int and 0 < len(vals) <= 25:   # integer scales only; float [0,1] outcomes fall through to proportion
            core = sorted(v for v in vals if v not in SENT and -90 <= v <= 90 and float(v).is_integer())
            cset = set(core)
            if core and len(cset) <= 2:
                return "binary"
            if core and min(core) >= 0 and max(core) <= 12 and len(cset) <= 12:
                return "ordinal"   # Likert / rating scale, robust to missing-value codes
        if len(vals) <= 2 and distinct <= 2:
            return "binary"
        if is_int and self.vmax is not None and self.vmax <= 12 and self.n_negative == 0 and distinct <= 12:
            return "ordinal"
        # count: a right-skewed non-negative integer EVENT tally of reasonable magnitude (King 1988's target).
        # requires positive skew (excludes symmetric integer covariates like age or 0-100 thermometers) and
        # caps magnitude (excludes integer-valued money/continuous quantities).
        if (is_int and self.n_negative == 0 and skew >= 1.0 and distinct > 4
                and self.vmax is not None and self.vmax < 1_000_000):
            return "count"
        if self.n_unit_interval == n:
            # A genuine bounded share/rate is (near-)continuous with many distinct values; a Likert or rating
            # item min-max-rescaled to [0,1] takes few discrete evenly-spaced values. The linear-model concern
            # is clear for the former (Papke-Wooldridge) and contested for the latter (ordinal-as-interval),
            # so route few-valued [0,1] to ordinal (debated tier), not clear-tier proportion. This is the
            # rescaled-ordinal failure class that drove the inter-rater disagreement (kappa 0.12).
            if distinct <= 12:
                return "ordinal"
            return "proportion"
        if self.n_negative == 0 and skew >= 2:
            return "heavy-tailed"
        return "continuous"
...
def build_jsonld_graph(
    ...
    return {
        # P2 extension: the base DDI-CDI 1.0 context plus a local term so distributionKind is a DEFINED
        # JSON-LD key (a conformant processor keeps it rather than dropping an undefined bare key).
        "@context": [DDI_CDI_CONTEXT, {"distributionKind": "https://rdm.libis.kuleuven.be/ns/cdi-ext#distributionKind"}],
